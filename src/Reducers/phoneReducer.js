...
export default phoneReducer = (state=INITIAL_STATE,actions) => {
    switch(actions.type)
    {
        ...
        case USER_BASIC_INFO_ALREADY_PRESENT:
            return {...state, userName : actions.payload.name, email : actions.payload.email, phoneNumber : actions.payload.phoneNumber, demoTaken : actions.payload.demoTaken, sending : false, sent : false}
        case USER_BASIC_INFO_NOT_PRESENT:
            return {...state, sending : false, sent : false}
        case USER_INFO_SENT_TO_SERVERS:
            return {...state, sending : true}
        case USER_REGISTERED : 
            return {...state, userName : actions.payload.name, email : actions.payload.email ,phoneNumber : actions.payload.phoneNumber, sending : false, sent : true, demoTaken : false }
        ...
        case DEMO_SESSION_BOOKED:
            return {...state, demoTaken : true}
        case ADD_SESSION:
            return {...state, streetName : actions.payload.streetName, houseNo : actions.payload.houseNo, landmark : actions.payload.landmark}
        default:
            return state;
    }
}
